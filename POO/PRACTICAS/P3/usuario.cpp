...
Clave::Clave(const char* p)
{
	...
	if(crypt(p,salt)==nullptr) throw Clave::Incorrecta(Clave::ERROR_CRYPT);	//ciframos y comprobamos que se haya realizado correctamente
	pass_c = crypt(p,salt);
}
...
{
	setlocale(LC_ALL,"");		//Elegimos como categoría la localidad total del programa y para localidad el entorno nativo
...
void mostrar_carro (ostream &os, const Usuario &u)
{
	...
	{
		os<<" "<<i->second<<"\t"<<"["<<i->first->referencia()<<"] \""<<i->first->titulo()<<"\", ";			//usamos iteradores para mostrar los elementos deseados
